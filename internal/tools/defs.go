...
// Risk classifies whether a tool needs approval in ask mode.
type Risk int

const (
	RiskSafe Risk = iota // auto in ask
	RiskWrite
	RiskShell
)

// ToolRisk returns the risk class for a tool name.
func ToolRisk(name string) Risk {
	switch name {
...
// KindFromTool maps tool name to a session step kind string used by UI.
func KindFromTool(name string) string {
	switch name {
	case "read_file":
		return "read"
	case "list_dir", "find_files":
		return "search"
	case "search_code":
		return "search"
	case "str_replace", "write_file":
